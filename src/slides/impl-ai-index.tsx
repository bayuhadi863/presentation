import screenshot from "../assets/fe-ai-index.png";
import { slideCount } from ".";
import { formatPageNumber } from "./utils";
import type { SlideProps } from "./types";

export const aiImplIndexId = "ai-impl-index";
export const aiImplIndexLabel = "AI: Index View";
export const aiImplIndexPageNumber = 10;
...
export default function AIImplIndexSlide({ mode }: SlideProps) {
  if (mode === "presentation") {
    return (
      <section
        className={presentationClass}
        aria-label="AI Implementation Index"
      >
        <div className="flex flex-col h-full max-w-7xl mx-auto w-full">
          <div className="mb-8">
            ...
            <h2 className="m-0 text-[clamp(1.8rem,3vw,2.8rem)] leading-tight tracking-[-0.04em] font-bold text-white">
              Module: Index View (Table Configuration)
            </h2>
          </div>
    ...
    >
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="text-sm text-sky-200/60">
          {formatPageNumber(aiImplIndexPageNumber, slideCount)}
        </span>
      </div>
